...
%%<<label=dplot, echo=FALSE, include=FALSE>>=
%%library(mvtnorm)
%%  x <- seq(-3, 3, length = 50)
%%  y <- x
%%  z<-matrix(0, 50, 50)
%%  mu<-c(0,0)
%%  sigma<-cbind(c(1,0.4),c(0.4,1))
%%  for(i in 1:50){
%%     for(j in 1:50){
...
%%persp(x, y, z, theta = 20, ltheta=60, phi = 30, lphi=0.3, expand = 0.5, col ="lightblue", shade=0.3, xlab="", ylab="", zlab="") 
%%text(-0.45,-0.11,expression(Pr))
%%text(-0.1,-0.35,expression(beta[1]))
%%text(0.3,-0.2,expression(beta[2]))
%%@




%%<<label=dplotgibbs, echo=FALSE, include=FALSE>>=
%%library(mvtnorm)
%%  x <- seq(0, 3, length = 50)
%%  y <- seq(-3, 3, length = 50)
%%  z<-matrix(0, 50, 50)
...
%%<<label=dplotMM, echo=FALSE, include=FALSE>>=
%%library(mvtnorm)
%%  x <- seq(-6, 6, length = 50)
%%  y <- seq(-6, 6, length = 50)
%%  z<-matrix(0, 50, 50)
...
%%  for(i in 1:50){
%%    for(j in 1:50){
%%      z[,i][j]<-0.5*dmvnorm(c(x[i], y[j]), mu1, sigma)+dmvnorm(c(x[i], y[j]), mu2, sigma)
%%    }
%%  }
%%par(bg="white")
%%xlab<-expression(beta)
...
%%text(-0.1,-0.35,expression(beta[1]))
%%text(0.3,-0.2,expression(beta[2]))
%%text(0.2,-0.15,expression(C[2]), col="white", cex=2)
%%text(-0.1,-0.25,expression(C[1]), col="white", cex=2)
%%@


%%<<label=conflik, echo=FALSE, include=FALSE>>=
%%library(mvtnorm)
%%  x <- seq(-3, 3,length = 50)
%%  y <- x
%%  z<-matrix(0, 50, 50)
...
%%  for(i in 1:50){
%%     for(j in 1:50){
%%      z[,i][j]<-dmvnorm(c(x[i], y[j]), mu<-c(x[i],x[i]), sigma)
%%    }
%%  }
%%par(bg="white")
%%xlab<-expression(beta)
%%persp(x, y, z, theta = -30, ltheta=60, phi = 30, lphi=0.3, expand = 0.5, col ="lightblue", shade=0.3, xlab="", ylab="", zlab="") 
%%text(-0.38,0.02,expression(Pr))
%%text(-0.27,-0.25,expression(beta[1]))
%%text(0.16,-0.33,expression(beta[2]))
%%@
...
%%model1<-MCMCped(PdP=PdPCervus, GdP=GdP, sP=sP, 
%%verbose=FALSE) 
%%@
...
%%<<echo=true, fig=FALSE>>=
%%sP<-startPed(estP=FALSE, dam=ped2[,2], sire=ped2[,3], estUSsire=FALSE)  
%%model2<-MCMCped(PdP=PdP, sP=sP, verbose=FALSE) 
%%plot(model2$beta)
%%summary(model2$beta)
%%@

%%<<label=model2beta, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model2$beta)
%%@
...
%%<<echo=true, fig=FALSE>>=
%%sP<-startPed(estG=FALSE,A=extractA(WarblerG),E2=0.005,estUSsire=FALSE,USsire=10)  
%%model3<-MCMCped(PdP=PdP, GdP=GdP, sP=sP, verbose=FALSE) 
%%plot(model3$beta)
%%summary(model3$beta)
%%@
 
%%<<label=model3beta, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model3$beta)
%%@
...
%%<<label=model4US, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model4$USsire)
%%@
...
%%<<echo=true>>=
%%MLEUSsire<-MLE.popsize(getXlist(PdP, GdP, E2=1e-10), 
...
%%<<echo=true>>=
%%MLEUSsire2<-MLE.popsize(getXlist(PdP, GdP, E2=0.01), 
%%USsire=TRUE, USdam=FALSE)
%%MLEUSsire2$nUS
%%# ML estimate
%%sqrt(MLEUSsire2$C)
...
We have no information regarding the phenotypes of unsampled males in the Seychelles Warbler, and therefore the data do not allow us to distinguish between a large unsampled population and a smaller population with a reduced chance of paternity.  However, as noted earlier, the discrepancy between our estimate of \Sexpr{round(median(model4$USsire),2)} unsampled males and our prior belief that the number of males should be less than 15 may be a consequence of underestimating the level of genotyping error.

\subsection{Genotyping Error}
\label{GE-sec}

At this point it will be usefull to distinguish between exact and approximate methods for dealing with genotyping error.  A useful notation will be $\bm{G}$ for true but unobserved genotypes, and $\bm{G}_{obs}$ for observed but possibly erroneus genotypes.  Both methods implicitly or explicitly use this concept, and the aim is to derive the probability distribution of $\bm{G}$ from $\bm{G}_{obs}$ and what we know about error rates ($\varepsilon$) and allele frequencies ($\bm{A}$).  The approximate method defines the conditional probability of $\bm{G}$ as
...
%%<<label=model5, echo=FALSE, fig=TRUE, include=FALSE>>=
%%E1<-model5$E1
%%E2<-model5$E2
%%USsire<-model5$USsire
%%plot(mcmc(cbind(E1,E2,USsire)))
%%@
...
%%model6<-MCMCped(PdP=PdP,GdP=GdP,pP=pP,tP=tunePed(USsire=0.1),verbose=FALSE) 
%%plot(model6$USsire)
%%@

%%<<label=model6US, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model6$USsire)
%%
%%@
...
%%# each individual is typed twice with high dropout
%%
%%tP<-tunePed(E1=15)
%%#MH tuning paramater needs to be larger
%%
...
%%ped[,1]<-1:100   
%%ped[,2][51:100]<-1:50 
%%# 50 dams and their offspring
%%
%%G<-simgenotypes(A=A, E1=0.1, E2=0.005, ped=ped, no_dup=1)
%%# only typed once
%%
%%sP<-startPed(dam=ped[,2])
%%#specify that the dams are known, 
%%#males are treated as base by default
%%
%%GdP<-GdataPed(G=G$Gobs, id=G$id)
...
%%<<label=model.pedEE2, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.pedE$E2)
%%@
...
%%# A new set of random genotypes with positive probability
%%# are generated and used as the starting configuration
%%model.config<-MCMCped(GdP=GdP,sP=sP,tP=tP,verbose=FALSE) 
%%plot(model.config$E2)
%%@

%%<<label=model.config, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.config$E2)
%%@
...
%%# 50 unrelated parent-offspring trios

%%G<-simgenotypes(A=A,E1=0.1,E2=0.005,ped=ped,no_dup=1)
%%sP<-startPed(dam=ped[,2], sire=ped[,3], G=G$G)

%%#specify that the dams and sires are known, 
%%#and use the true G as the starting configuration
...
%%#use a random G as the starting configuration
...
%%<<label=model.DSconfig, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(c(model.DS1$E2), type="l", col="blue",ylim=c(0,post.max))
%%lines(c(model.DS2$E2), type="l", col="red")
%%@
...
%%
%%G<-simgenotypes(A=A, E1=0.005, E2=0.005, ped=simped$ped, no_dup=1)
%%GdP<-GdataPed(G=G$Gobs, id=G$id)
%%
...
%%# identical models but the 2nd and 3rd models 
%%# exclude parents with 3 or more, or 2 or more,
%%# mismatches, repectively
%%
...
%%counts[as.numeric(names(noff))]<-counts[as.numeric(names(noff))]+noff 
%%# number of offspring per mother
...
%%
%%PdP<-PdataPed(formula=list(res1, var1, var2, list(var1, var2)), data=data_INT)
...
Markov chains may not mix well when the posterior distribution of the parameters are not independent. Indeed, the autocorrelation approaches 1 despite a thinning interval of 10 (See Figure \ref{INT-fig}).
...
%%<<label=model.INT, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.INT$beta)
%%@
...
%%<<label=model.INT.cntr, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.INT.cntr$beta)
%%@
...
%%data_EPP<-as.data.frame(list(id=1:175, sex=sex,
%% offspring=offspring, terr=terr))
...
%%G<-simgenotypes(A=A,E1=0.01,E2=0.01,ped=simped$ped,no_dup=1)
%%GdP<-GdataPed(G=G$Gobs,id=G$id)

%%# remove unsampled males 

%%rm_males<-1:25

%%data_EPP_miss<-data_EPP[-rm_males,]
%%GdP_miss<-GdataPed(G=lapply(G$Gobs, function(x){x[-rm_males]}), id=G$id[-rm_males])
...
%%<<label=model.miss, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.miss$beta)
%%@
...
%%<<label=model.missUS, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.miss$USsire)
%%@
...
%%# unsampled males are presumed to be present in 
%%# the same ratios so do not enter into the equation

%%theta_o<-(NW*theta)/(NW*theta+NE*(1-theta))
%%plot(mcmc(theta_o))
%%@

%%<<label=model.missTest, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(mcmc(theta_o))
%%@
...
%%<<label=model.miss2, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.miss2$beta)
%%@
...
%%<<label=model.miss2US, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.miss2$USsire)
%%@
...
%%<<label=EPP2Test, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(mcmc(theta_o))
%%@
...
%%<<echo=TRUE, fig=FALSE>>=
%%var3<-expression(varPed(x="terr", gender="Male", 
%%relational="OFFSPRING", USvar=FALSE, merge=TRUE))
%%PdP_miss3<-PdataPed(formula=list(res1, var3, res2), 
%%data=data_EPP_miss, USsire=TRUE)
%%model.miss3<-MCMCped(PdP=PdP_miss3, GdP=GdP_miss, 
%%verbose=FALSE)
%%theta_o<-inv.logit(model.miss3$beta)
%%plot(mcmc(theta_o))
%%@

%%<<label=EPPmerge, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(mcmc(theta_o))
%%@
...
%%table(MT[as.numeric(simped$ped[,2])]==MT[as.numeric(simped$ped[,3])])

%%# offspring produced by +/- unions should make up (on average) a
%%# quarter of the total offspring, and offspring produced by +/+
%%# and -/- unions should make up the remaining 75%%%.  Note this is
%%# only because the frequency of + and - mating types are equal
...
%%<<label=model.ass.mat, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(mcmc(inv.logit(model.ass.mat$beta)))
%%@
...
%%res1<-expression(varPed(x="offspring", restrict=0))
%%res2<-expression(varPed(x="year", relational="OFFSPRING", restrict="=="))

%%#parents must be recorded in the year the offspring was born
%%#but must not be born in that year

%%var1<-expression(varPed(x=c("lat", "long"), lag=c(0,0), relational="OFFSPRING"))
%%# both maternity and paternity is a function of distance beween parents
%%# and offspring in the year the offspring was born.

%%PdP<-PdataPed(formula=list(res1, res2, var1), data=data.ped, timevar=year)
...
%%<<label=MGped, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.ped$beta)
%%@
...
%%<<label=Herm, echo=FALSE, fig=TRUE, include=FALSE>>=
%%impb<-mcmc(inv.logit(model.herm$beta))
%%names(impb)<-"inv.logit.beta"
%%plot(impb)
%%@
...
%%<<label=mon1, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.mon$beta)
%%@
...
%%<<label=mon2, echo=FALSE, fig=TRUE, include=FALSE>>=
%%plot(model.mon2$beta)
%%@

              ...
              for(itt_set= 0; itt_set < nl; itt_set++){
               ...
               if(set_size==2){        
                ...
                for(ns=0; ns < n; ns++){               // itterate through samples 
                   sample_cat = cat_tmp[ns];
                   if(obs_G[ns][0]==obs_G[ns][1]){      // if obs is homozygote
                      hom1=0;
                      if(obs_G[ns][0]==a2){hom1=1;}     // if homozygous for second allele
                      Ppart[0] *= E_mat[l][sample_cat+1];          //  a1/a2
                      Ppart[1] *= E_mat[l][sample_cat+(hom1*2)];   //  a1/a1 if(obs=a1/a1 e_a, if not e_c)   
                      Ppart[2] *= E_mat[l][sample_cat+2-(hom1*2)]; //  a2/a2 if(obs=a2/a2 e_a, if not e_c)   
                      Ppart[3] *= E_mat[l][sample_cat+1+hom1];     //  a1/- if(obs=a1/a1 e_b, if not e_c)   
                      Ppart[4] *= E_mat[l][sample_cat+2-hom1];     //  a2/- if(obs=a2/a2 e_b, if not e_c)
                      Ppart[5] *= E_mat[l][sample_cat+2];          // -/-
                   }else{                                   // if obs is heterozygote
                      Ppart[0] *= E_mat[l][sample_cat+3];          
                      Ppart[1] *= E_mat[l][sample_cat+4];
                      Ppart[2] *= E_mat[l][sample_cat+4];
                      Ppart[3] *= E_mat[l][sample_cat+5];
                      Ppart[4] *= E_mat[l][sample_cat+5];
                      Ppart[5] *= E_mat[l][sample_cat+6];
                 ...
                 for(ns=0; ns < n; ns++){
                    ...
                    for(itt_set= 0; itt_set < set_size; itt_set++){
                     na2 = unique_set[itt_set];
                       for(na1= 0; na1 < nl; na1++){ 
                           if(a1==a2){  
                               eterm = E_mat[l][1+sample_cat];                        
                              if(na1==a1 && na2==a1){ 
                               eterm = E_mat[l][sample_cat];
                              }
                              if(na1!=a1 && na2!=a1){
                               eterm = E_mat[l][2+sample_cat];
                              } 
                           }else{   
                              eterm = E_mat[l][5+sample_cat];
                              if(na1==na2){
                  ...
                  if(a1==a2){ 
                   Ppart[set_size*nl] *= E_mat[l][2+sample_cat];
                  }else{ // in those cases where the true genotype does not contain sampled alleles
                   Ppart[set_size*nl] *= E_mat[l][6+sample_cat];
                  } 
